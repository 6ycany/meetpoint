...
| プロジェクト設計・仕様策定 | ✅ 完了 |
| Androidプロジェクト雛形 | ✅ 完了 |
| UI実装（Jetpack Compose） | ✅ 完了 |
| 地図表示（Google Maps SDK） | ✅ 完了 |
| 住所検索（Geocoding API） | ✅ 完了 |
| ルート距離計算（Routes API） | ⬜ 未着手（Phase 2） |
| SA/PA検索（Overpass API） | ✅ 完了 |
| 駅検索（Overpass API） | ✅ 完了 |
| モード1：集合のみ計算ロジック | ✅ 完了 |
| モード2：合流→目的地計算ロジック | ⬜ 未着手（Phase 3） |
| 結果候補3件表示 | ✅ 完了 |
| 機能1：現在地ワンタップ入力 | ⬜ 未着手（Phase 3） |
| 機能2：Google Mapsナビ連携 | ✅ 完了 |
| 機能3：セッション履歴保存（Room DB） | ⬜ 未着手（Phase 3） |
| 機能5：不公平度スコア可視化（棒グラフ・A〜Dランク） | ⬜ 未着手（Phase 3） |
